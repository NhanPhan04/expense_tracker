...
useEffect(() => {
  api.get("/admin/me")
    .then(res => {
      const user = res.data.data || {}; // fallback
      setAdmin(prev => ({
        ...prev,
        name: user.name || "",
        email: user.email || "",
  ...
  const handleAvatarChange = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    setAdmin(prev => ({ ...prev, avatarFile: file }));
  };

  // Upload avatar và update thông tin
  const handleUpdate = async () => {
    try {
      ...
      if (admin.avatarFile) {
        ...
        formData.append("avatar", admin.avatarFile);

        const res = await api.post("/admin/avatar", formData, {
      ...
      setAdmin(prev => ({ ...prev, avatar: avatarUrl, avatarFile: undefined }));
      setPreview(null);
    } catch (err) {
      console.error(err);
      alert("Có lỗi xảy ra!");
